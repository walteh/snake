...
func callRunMethod(cmd *cobra.Command, f reflect.Value, t reflect.Type) error {
	...
	for _, pt := range listOfArgs(t) {
		...
		if !contextOverrideExists && pt.Implements(reflect.TypeOf((*context.Context)(nil)).Elem()) {
			...
			if !bindingsExist {
				return terrors.Wrapf(ErrMissingBinding, "no snake bindings in context, looking for type %q", pt)
			}
...
func commonValidateRunMethod(inter any, method reflect.Value) (reflect.Type, string, error) {
	...
	if method.Kind() == reflect.Invalid || method.IsZero() || method.IsNil() {
		return nil, parentName, terrors.Wrapf(ErrMissingRun, "target ===> %s", parentName)
	}

	if !method.IsValid() {
		return nil, parentName, terrors.Wrapf(ErrInvalidRun, "target ===> %s", parentName)
	}

	if method.Kind() != reflect.Func {
		return nil, parentName, terrors.Wrapf(ErrInvalidRun, "expected function, got %s for (%s).Run", method.Type(), parentName)
	}
...
func validateRunMethod(inter any, method reflect.Value) (reflect.Type, error) {
	...
	if t.NumOut() != 1 || !t.Out(0).Implements(callbackReturnSignature) {
		return nil, terrors.Wrapf(ErrInvalidRun, "return value of (%s).Run must be of type \"error\"", pname)
	}
